...
2. Create a `.env.local` file and fill in the necessary environment variables:
...
## Static export / S3 deployment

This project includes an API route (`/api/token`) for local development, which is **not compatible** with Next.js static export.

- **Local dev**: use `pnpm dev` (default)
- **Static export**: only enable if you have removed API routes and are using an external token server.
  When ready, run:
