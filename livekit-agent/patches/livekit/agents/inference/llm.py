...
from ..types import DEFAULT_API_CONNECT_OPTIONS, NOT_GIVEN, APIConnectOptions, NotGivenOr, FlushSentinel
...
class LLMStream(llm.LLMStream):
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            chunk_counter = 0  # DEBUG: Track chunk count
            flush_sent = False  # Track if we've sent FlushSentinel for tool_calls
            # Buffer small deltas so downstream TTS gets meaningful chunks.
            # This prevents ElevenLabs from waiting until tool result arrives,
            # which makes filler + tool response sound "glued together".
            text_buf = ""
            # Larger threshold = less chance filler and tool result get synthesized together.
            TEXT_FLUSH_MIN_CHARS = 100
            async with stream:
                async for chunk in stream:
                    chunk_counter += 1
                    # DEBUG: Log every chunk received from LLM
                    saw_tool_calls_finish = False
                    for choice in chunk.choices:
                        delta_content = choice.delta.content if choice.delta else None
                        finish_reason = choice.finish_reason
                        logger.info(
                            f"[CHUNK #{chunk_counter}] content='{delta_content}' | finish_reason={finish_reason}"
                        )
                    ...
                    for choice in chunk.choices:
                        ...
                        if chat_chunk is not None:
                            retryable = False
                            # If this chunk contains tool-call deltas, flush buffered text first,
                            # then forward the tool-call chunk as-is.
                            if chat_chunk.delta and chat_chunk.delta.tool_calls:
                                if text_buf:
                                    logger.info(f"[SENDING TO TTS] '{text_buf}'")
                                    self._event_ch.send_nowait(
                                        llm.ChatChunk(
                                            id=chunk.id,
                                            delta=llm.ChoiceDelta(content=text_buf, role="assistant"),
                                        )
                                    )
                                    text_buf = ""
                            ...
                            # Buffer assistant text so we don't stream token-by-token to TTS.
                            if chat_chunk.delta and chat_chunk.delta.content:
                                text_buf += chat_chunk.delta.content
                                if (
                                    len(text_buf) >= TEXT_FLUSH_MIN_CHARS
                                    or text_buf.endswith((".", "?", "!", "\n"))
                                ):
                                    logger.info(f"[SENDING TO TTS] '{text_buf}'")
                                    self._event_ch.send_nowait(
                                        llm.ChatChunk(
                    ...
                    # FILLER FIX: If tool_calls is detected, force-flush any buffered filler text,
                    # then emit FlushSentinel so TTS can start playback before tool result arrives.
                    if saw_tool_calls_finish and not flush_sent:
                        if text_buf:
                            logger.info(f"[SENDING TO TTS] '{text_buf}'")
                            self._event_ch.send_nowait(
                                llm.ChatChunk(
                            ...
                            text_buf = ""
                        flush_sent = True
                        logger.info("[FLUSH] Emitting FlushSentinel - tool_calls detected, flushing TTS")
