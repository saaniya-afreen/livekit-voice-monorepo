...
class LLMStream(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        self._event_ch = aio.Chan[Union[ChatChunk, FlushSentinel]]()
    ...
    async def _metrics_monitor_task(self, event_aiter: AsyncIterable[ChatChunk]) -> None:
        ...
        async for ev in event_aiter:
            # Skip FlushSentinel - it's not a ChatChunk and has no .id attribute
            if isinstance(ev, FlushSentinel):
                continue
    ...
    async def __anext__(self) -> ChatChunk | FlushSentinel:
        try:
    ...
    def __aiter__(self) -> AsyncIterator[ChatChunk | FlushSentinel]:
        return self
